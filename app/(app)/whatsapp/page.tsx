      ...
      <ul className="mt-10 space-y-4">
        {coursesWithSteps.map((course, index) => (
          <li key={course.id} className="animate-sprout-in" style={{ animationDelay: `${index * 80}ms` }}>
            <Link href={`/whatsapp/${course.id}`} className="glass btn-glow block rounded-[var(--radius-lg)] p-5">
        ...
        {coursesWithSteps.length === 0 && (
          <p className="text-ink-faint">Todavía no hay cursos con un flujo de WhatsApp configurado.</p>
        )}
      </ul>
    </div>
  );
